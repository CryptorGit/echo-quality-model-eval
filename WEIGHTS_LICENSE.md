# Model Weights License (Non-Commercial)

This repository includes pretrained model weight files ("Weights").

## Permission (Non-Commercial Only)
You may download and use the Weights **only for non-commercial purposes**, such as research, education, and evaluation.

Commercial use is **not permitted**, including (but not limited to) use in products/services, paid consulting, or any activity primarily intended for commercial advantage or monetary compensation.

## Dataset / Data Terms
Use of the Weights and any results derived from them may be **restricted by the license/terms of the underlying datasets** used for training and evaluation.

If the dataset terms applicable to you do **not** allow such use, you must not use the Weights.

If the underlying dataset terms do **not** allow redistribution of derivatives (including model weights), you must **delete the Weights** from your local copy and only use this repository as a code reference / for re-training under compliant terms.

## No Warranty
The Weights are provided **as-is**, without warranties of any kind. You are responsible for validating performance and suitability for your use case.

## Medical / Clinical Use
The Weights are provided for research and evaluation. **Do not use** them for clinical decision making.

## Redistribution
If you redistribute the Weights, you must keep this file and the non-commercial restriction intact.

You are responsible for ensuring redistribution is permitted by the underlying dataset terms.
