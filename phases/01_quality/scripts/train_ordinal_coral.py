...
def ordinal_targets_int(y_int: torch.Tensor) -> torch.Tensor:
    # y_int: [B] in [0..9] -> targets for k=1..9
    bins = torch.arange(1, 10, device=y_int.device, dtype=y_int.dtype).unsqueeze(0)
    return (y_int.unsqueeze(1) >= bins).float()


class Phase1OrdinalDataset(Dataset):
...
def evaluate(model: nn.Module, loader: DataLoader, device: str, amp: bool) -> dict:
    ...
    for b in loader:
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(amp and device == "cuda")):
            logits = model(x, vid)
        pred_cont = torch.sigmoid(logits.float()).sum(dim=1).detach().cpu().numpy()
        pred_int = np.clip(np.rint(pred_cont), 0, 9).astype(int)
...
def train(cfg: Cfg):
    ...
    ds_tr = Phase1OrdinalDataset(tr_csv, cfg, train=True)
    ds_va = Phase1OrdinalDataset(va_csv, cfg, train=False, view_to_idx=ds_tr.view_to_idx)
    ds_te = Phase1OrdinalDataset(te_csv, cfg, train=False, view_to_idx=ds_tr.view_to_idx)
    ...
    model = ResNet50CORAL(n_views=len(ds_tr.view_to_idx), view_emb_dim=cfg.view_emb_dim).to(device).to(memory_format=torch.channels_last)
    ...
    for ep in range(1, cfg.epochs + 1):
        model.train()
        pbar = tqdm(dl_tr, desc=f"coral ep {ep}/{cfg.epochs}")
        total_loss = 0.0
        n = 0
...
def main():
    ...
    p.add_argument("--run_tag", default="coral_resnet50")
